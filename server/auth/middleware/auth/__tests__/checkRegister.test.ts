...
import { checkRegister } from "../authChecks";
...
test("Testing middleware checkRegister to see that it fails with invalid phone.", (done) => {
  request(app)
    .post("/register")
    .send({
      fName: "john",
    ...
    .set("Accept", "application/json")
    .expect(400)
    .expect({ msg: "Please put in a valid phone number." })
    .end(function (err, res) {
      if (err) return done(err);
